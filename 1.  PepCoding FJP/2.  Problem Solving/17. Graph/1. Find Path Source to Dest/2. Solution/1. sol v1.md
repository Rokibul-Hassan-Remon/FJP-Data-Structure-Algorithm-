```java title:"Find the path from source to destination"
public class Edge {  
    int source;  
    int neighbour;  
    int weight;  
...
public class Main {  
    public static boolean HasPath(ArrayList<Edge>[] graph, int src, int dest, boolean[] isVisited){  
        ...
        for(var each : graph[src]){  
            if(isVisited[each.neighbour] == false){  
                boolean pathFindOrNot = HasPath(graph, each.neighbour, dest, isVisited);  
                if(pathFindOrNot == true){  
                    return  true;  
                }  
            }  
        }  
    ...
    public static void main(String[] args) throws Exception {  
		...
		boolean PathFindOrNot = HasPath(adjacencyList, stratingPoint, endingPoint, isVisited);  
		Println("Source ->"+stratingPoint+ " to Destination ->"+endingPoint+ " Has Path ? "+ (PathFindOrNot? "Yes" :"No"));
    }  
